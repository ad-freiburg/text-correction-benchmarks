...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "in_file", type=str, help="Path to the input file containing misspelled text.")
    parser.add_argument(
        "gt_file", type=str, help="Path to the groundtruth file containing the target outputs.")
    parser.add_argument(
        "pred_file",
        type=str,
...
def evaluate(
    ...
    for name in sorted(metric_names):
        if name == "bin_f1":
            binary_predictions = [
                bool(int(p)) for prediction in predictions for p in prediction.split()]
            binary_labels = [bool(int(lab))
                             for label in groundtruths for lab in label.split()]
            f1, prec, rec = metrics.binary_f1(
                binary_predictions, binary_labels
            )
            print(
                f"F1: {100 * f1:.2f} (Precision: {100 * prec:.2f}%, Recall: {100 * rec:.2f}%)"
            )

        elif name == "word_acc":
            word_predictions = [
                p for prediction in predictions for p in prediction.split()]
            word_groundtruths = [
                lab for label in groundtruths for lab in label.split()]
